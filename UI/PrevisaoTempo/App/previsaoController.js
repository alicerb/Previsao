PrevisaoApp.controller('previsaoController', ['$scope', '$location', 'previsaoService', function ($scope, $location, previsaoService) {

    $scope.initPrevisao = function () {

        $scope.cidades = {};
        $scope.loading = false;
        getPrevisao($scope, $location, previsaoService);

    }

}]);

function getPrevisao($scope, $location, previsaoService) {

    $scope.loading = true;
    previsaoService.request('/Previsao/ObterPrevisao?idAPI=' + $location.search().idAPI, 'Get', null).then(function (result) {
...
renderData = (forecast) => {
  ...
  ${Math.round(currentWeather.Temp) + '°C'} <i class="wi wi-degrees"></i>`;
    ...
    forecast.forEach(day => {
      ...
      <span class="degrees">Máx:${Math.round(day.TempMax) + '°C / Min:' + Math.round(day.TempMin) + '°C'}
